...
const App = () => {
  ...
  function addNewOrder(){ // função para adicionar o pedido com id, pedido e nome
    setClientOrders([... clientOrders,{id: Math.random(), order: inputOrder.current.value, name: inputName.current.value}]) // uso de spread operator (...) fazendo acrescentar o array antigo + item novo
  }

  function deleteOrder(orderId){ // função para deletar o pedido
